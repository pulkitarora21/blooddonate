...
OZ(up,"zBh",xwJ);OZ(up,"GetSourceFrameRateFPS",xwJ);
function xwJ(){
return-1.0;
}
...
IcH.X1E.a9(IcH.FVe,IcH.QG);IcH.DDV.a9(IcH.QG,IcH.FVe);
...
if(up==fj)StreamerStateManager.yeD="PlayingStateChange";
if(up==fj)StreamerStateManager.xUm="StreamInfoChange";
if(up==fj)StreamerStateManager.pOB="AvailableStreamInfoChange";
if(up==fj)StreamerStateManager.vHz="ErrorChange";
if(up==fj)StreamerStateManager.GMW="MetadataChange";
if(up==fj)StreamerStateManager.kTl="LoadingStatusChange";
if(up==fj)StreamerStateManager.zJ2="DisplaySizeChange";
if(up==fj)StreamerStateManager.x1x="Log";
...
VW8(StreamerStateManager.kTl,Nch);
}
}
}
...
VW8(StreamerStateManager.yeD,Uy);
}
}
up.OjE=Uy;
}
...
VW8(StreamerStateManager.xUm,Wiw);
}
}
up.UQZ=Wiw;
}
...
VW8(StreamerStateManager.vHz,lNq);
}
}else{
up.FvG.Yb(lNq);
}
...
VW8(StreamerStateManager.GMW,UFr);
}
}
...
VW8(StreamerStateManager.pOB,Xdp);
}
}
}
...
VW8(StreamerStateManager.zJ2,Gm8);
}
}
}
...
VW8(StreamerStateManager.x1x,message);
}
...
VW8(StreamerStateManager.x1x,"ERROR:"+message);
}
}

dN(up,StreamerStateManager,"REASON_AVAILABLESTREAMINFOCHANGE",StreamerStateManager.pOB);
dN(up,StreamerStateManager,"REASON_DISPLAYSIZECHANGE",StreamerStateManager.zJ2);
dN(up,StreamerStateManager,"REASON_ERRORCHANGE",StreamerStateManager.vHz);
dN(up,StreamerStateManager,"REASON_LOADINGSTATUSCHANGE",StreamerStateManager.kTl);
dN(up,StreamerStateManager,"REASON_LOG",StreamerStateManager.x1x);
dN(up,StreamerStateManager,"REASON_METADATACHANGE",StreamerStateManager.GMW);
dN(up,StreamerStateManager,"REASON_PLAYINGSTATECHANGE",StreamerStateManager.yeD);
dN(up,StreamerStateManager,"REASON_STREAMINFOCHANGE",StreamerStateManager.xUm);
if(up!=fj)ju.apply(up,arguments);
}
Bg(StreamerStateManager,"StreamerStateManager");
...
if(up==fj)Uj.tj=74;
if(up==fj)Uj.pb=0;
if(up==fj)Uj.ya=14840;


if(up==fj)Uj.bxs="Conviva LivePass version 2.74.0.14840";
...
ct.qF("Ms["+up.Bl+"]","Start lightMonitor for resource "+(sr!=null?sr:"null"));
...
O9.y8(up.ta.tc(0).w6.wf,"fakeSelectResponse:hasString");
ve3=up.ta.tc(0).w6.C6.T3();
}else{
}
...
LK(up,"tso",DIc);
function DIc(RoH,SVh){

ct.qF("MonitorCSP","notificationFromStreamerStateManager "+RoH);
...
},"notificationFromStreamerStateManager");
}
